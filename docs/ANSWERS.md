...
Au niveau du développement local, on peut utiliser un **venv** pour compartimenter les dépendances du projet, puis exécuter les fichiers ETL un à un pour valider leur fonctionnement.
...
3. **test_load.py** :
   - `test_save_data_as_csv` : Vérifie que la fonction **save_data_as_csv** crée le répertoire spécifié pour stocker les fichiers **CSV** en utilisant **os.makedirs** et qu'elle sauvegarde chaque **DataFrame** dans un fichier **CSV** avec le bon nom de fichier.
...
Bien que ces tests dépassent la portée du test technique demandé, j'y ai réfléchi lors de la conception et du développement du projet.
...
2. **Calcul des recommandations (Python, Airflow, MLflow)** :
   - Utilisation de **modèles hybrides** (filtrage collaboratif, basé sur le contenu) pour générer les recommandations (ex. combinaison avec un modèle de régression).
